...
# 工件1
# 可更改参数列表:
# 梯度标记边缘canny(下阈值canny_low , 上阈值canny_high)
# 高斯模糊Gauss(卷积核大小ksize)
# 二值化算法otsu(阈值边界thresh)
# 形态学内核(shape)
# 霍夫圆检测范围(minDist, minRadius, maxRadius)

# 预设值:工件1 (180, 200, 9, 0, 3, 1000, 300, 500)
# 预设值:工件3 (200, 250, 9, 150, 3, 1000, 300, 400)
...
def print_hi(path):
    ...
    ret1, binary_otsu = cv2.threshold(gauss, 150, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

    kernel = np.ones((3, 3), np.uint8)
    erosion = cv2.dilate(binary_otsu, kernel)
    # erosion = cv2.erode(binary_otsu, kernel)
